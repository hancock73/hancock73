...
def info_by_id():
    clear()
    user_id = input(color.Yellow + 'persik_cyk' + color.Cyan)
    color.END
    ...
    link = 'https://vk.com/persik_cyk' + str(info.get('id'))
    ...
    relation_list = ['не указано', 'не женат', 'не замужем', 'есть друг', 'есть подруга', 'помолвлен', 'помолвлена',
    ...
    if relation == 1:
        if male == 2:
            ...
            show_r = str(relation_list[2])
        sp = str('Половинка: нет')
    elif relation == 2:
        if male == 2:
            show_r = str(relation_list[3])
        else:
            show_r = str(relation_list[4])
        try:
        ...
        if male == 2:
            show_r = str(relation_list[11])
        else:
            show_r = str(relation_list[12])
        try:
            rp = dict(info.get('relation_partner'))
            passed = 1
        except TypeError:
...
def dl_history():
    clear()
    print(
        color.Cyan + "Данная функция доступна в нашем Телеграм боте! : @Vk_Tokenner_bot ! Для выхода в главное меню нажмите Q" + color.END)
    choice = input('>>> ')
...
def page_info():
    clear()
    print(
        color.Yellow + '[' + color.Cyan + '1' + color.Yellow + '] Записать в profile_info.txt [' + color.Cyan + '0' + color.Yellow + '] Вывести в командную строку\n' + color.END)
    ...
    pro = ['не указано', 'не женат', 'не замужем', 'есть друг', 'есть подруга', 'помолвлен', 'помолвлена', 'женат',
           'замужем', 'всё сложно', 'в активном поиске', 'влюблён', 'влюблена', 'в гражданском браке', ]
...
while infinite < 5:
    ...
    if cmd == '0':
        ...
        print(color.Green + 'Некорректный ввод!' + color.END)
